    ...
    <div ref={pageRef} className="sa-cars-page sa-catalogue--cars min-h-screen pb-24 transition-colors duration-200">
      <section className="sa-cars-hero">
        ...
        <div className="sa-cars-hero__content">
          ...
          <h1>Luxury moves<br /><em style={{ color: '#c6a15b' }}>differently.</em></h1>
      ...
      <section id="cars-collection" className="sa-cars-collection sa-cars-wrap">
        <div className="sa-cars-section-heading">
          <div>
        ...
        <div className="sa-cars-filter-rail">
          <label>
            ...
            <input
              ...
              style={{ borderColor: '#e1e1e1', color: '#b6b6b6' }}
            />
            {keyword && <button onClick={() => setKeyword('')} aria-label="Clear search"><X size={15} /></button>}
          </label>
          <div className="sa-cars-categories">
            {categories.map(c => (
              <button
                key={c.value}
                onClick={() => setCategory(c.value)}
                style={category !== c.value ? { color: '#c0c0c0' } : undefined}
      ...
      <section className="sa-cars-process">
        <div className="sa-cars-wrap">
          <p className="sa-cars-eyebrow">The simple route</p>
          <div className="sa-cars-process__head">
            <h2>From browsing<br /><em style={{ color: '#c6a15b' }}>to moving.</em></h2>
            <p>Every vehicle page keeps the next step clear: inspect the details, choose your dates, and send an enquiry.</p>
          </div>
          <div className="sa-cars-steps">
            <div><span>01</span><h3>Browse the fleet</h3><p style={{ backgroundColor: '#000000', color: '#ffffff' }}>Explore the current collection by category, brand or model.</p></div>
            <div><span>02</span><h3>Select a vehicle</h3><p style={{ color: '#ffffff' }}>Open the vehicle dossier for verified rental information and available details.</p></div>
            <div><span>03</span><h3>Send your enquiry</h3><p style={{ color: '#ffffff' }}>Use WhatsApp or the reservation form to share your dates and requirements.</p></div>
          </div>
        </div>
      </section>

      <section className="sa-cars-media">
        <div className="sa-cars-media__image" style={{ backgroundImage: `url(${heroVehicle?.gallery?.[1] || heroVehicle?.mainImage || 'https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=2200&q=85'})` }} />
        <div className="sa-cars-media__copy">
          <p className="sa-cars-eyebrow">The road ahead</p>
          <h2>Arrive with<br /><em style={{ color: '#c6a15b' }}>intention.</em></h2>
        </div>
      </section>

      <section className="sa-cars-cta">
        <div>
          <p className="sa-cars-eyebrow">Private vehicle enquiries</p>
          <h2>Ready when<br /><em style={{ color: '#c6a15b' }}>you are.</em></h2>
          <p>Tell us which vehicle caught your eye and when you need it. Our existing enquiry and WhatsApp channels are ready when you are.</p>
          <div className="sa-cars-hero__actions">
            <a href={whatsappUrl} target="_blank" rel="noreferrer" className="sa-cars-button sa-cars-button--light"><MessageSquare size={16} /> Talk on WhatsApp</a>
            <button onClick={() => navigate('/contact')} className="sa-cars-hero__link">Contact the team <ArrowRight size={16} /></button>
          </div>
        </div>
      </section>
    </div>
