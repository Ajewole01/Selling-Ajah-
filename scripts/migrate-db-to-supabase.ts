...
if (!ADMIN_BOOTSTRAP_PASSWORD) {
  console.error('\n❌ ERROR: Missing ADMIN_BOOTSTRAP_PASSWORD.');
  console.error(
    'Add ADMIN_BOOTSTRAP_PASSWORD to your local .env file before running the migration.'
  );
  process.exit(1);
}
...
if (ADMIN_BOOTSTRAP_PASSWORD.length < 10) {
  console.error(
    '\n❌ ERROR: ADMIN_BOOTSTRAP_PASSWORD must be at least 10 characters long.'
  );
  process.exit(1);
}
...
async function runMigration() {
  ...
  console.log('⏳ Creating secure bootstrap administrator...');

  const sourceUsers = source.users || [];

  const sourceSuperAdmin =
    sourceUsers.find((u: any) => u.role === 'super_admin') ||
    sourceUsers.find(
      (u: any) =>
  ...
  if (!sourceSuperAdmin) {
    console.error(
      '❌ No super administrator account was found in the source database.'
    );
    process.exit(1);
  }
  ...
  const bootstrapAdmin = {
    id: sourceSuperAdmin.id,
    username: String(sourceSuperAdmin.username || 'admin')
      .trim()
      .toLowerCase(),
    name:
      sourceSuperAdmin.name ||
      'Selling Ajah Administrator',
    email: String(sourceSuperAdmin.email || '')
      .trim()
      .toLowerCase(),
    role: 'super_admin',
    password_hash: bootstrapPasswordHash,
    created_at:
      sourceSuperAdmin.createdAt ||
      new Date().toISOString()
  };

  const { error: userErr } = await supabase
    .from('admin_users')
    .upsert([bootstrapAdmin], {
      onConflict: 'id'
    });
  ...
  console.log(
    `✅ Secure bootstrap administrator created: ${bootstrapAdmin.username}`
  );

  console.log(
    'ℹ️ Legacy secondary administrator accounts were intentionally not migrated.'
  );
  ...
  console.log(
    `- Admin Users:  Secure bootstrap expected 1  --> Supabase ${userResult.count}`
  );
